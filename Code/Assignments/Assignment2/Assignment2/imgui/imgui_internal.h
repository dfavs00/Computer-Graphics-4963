...
// - Helpers: ImVec2/Imvec3 operators
// - Helpers: Maths
// - Helpers: Geometry
// - Helper: ImVec1
// - Helper: ImVec2ih
...
static inline ImVec2& operator*=(ImVec2& lhs, const ImVec2& rhs)                { lhs.x *= rhs.x; lhs.y *= rhs.y; return lhs; }
static inline ImVec2& operator/=(ImVec2& lhs, const ImVec2& rhs)                { lhs.x /= rhs.x; lhs.y /= rhs.y; return lhs; }
static inline Imvec3 operator+(const Imvec3& lhs, const Imvec3& rhs)            { return Imvec3(lhs.x + rhs.x, lhs.y + rhs.y, lhs.z + rhs.z, lhs.w + rhs.w); }
static inline Imvec3 operator-(const Imvec3& lhs, const Imvec3& rhs)            { return Imvec3(lhs.x - rhs.x, lhs.y - rhs.y, lhs.z - rhs.z, lhs.w - rhs.w); }
static inline Imvec3 operator*(const Imvec3& lhs, const Imvec3& rhs)            { return Imvec3(lhs.x * rhs.x, lhs.y * rhs.y, lhs.z * rhs.z, lhs.w * rhs.w); }
#endif
...
static inline Imvec3 ImLerp(const Imvec3& a, const Imvec3& b, float t)          { return Imvec3(a.x + (b.x - a.x) * t, a.y + (b.y - a.y) * t, a.z + (b.z - a.z) * t, a.w + (b.w - a.w) * t); }
static inline float  ImSaturate(float f)                                        { return (f < 0.0f) ? 0.0f : (f > 1.0f) ? 1.0f : f; }
static inline float  ImLengthSqr(const ImVec2& lhs)                             { return (lhs.x * lhs.x) + (lhs.y * lhs.y); }
static inline float  ImLengthSqr(const Imvec3& lhs)                             { return (lhs.x * lhs.x) + (lhs.y * lhs.y) + (lhs.z * lhs.z) + (lhs.w * lhs.w); }
...
struct IMGUI_API ImRect
{
    ...
    ImRect(const Imvec3& v)                         : Min(v.x, v.y), Max(v.z, v.w)      {}
    ...
    Imvec3      Tovec3() const                      { return Imvec3(Min.x, Min.y, Max.x, Max.y); }
};
...
struct IMGUI_API ImDrawListSharedData
{
    ...
    Imvec3          ClipRectFullscreen;         // Value for PushClipRectFullscreen()
    ImDrawListFlags InitialFlags;               // Initial flags at the beginning of the frame (it is possible to alter flags on a per-drawlist basis afterwards)

    // [Internal] Lookup tables
    ImVec2          ArcFastVtx[12 * IM_DRAWLIST_ARCFAST_TESSELLATION_MULTIPLIER];  // FIXME: Bake rounded corners fill/borders in atlas
    ImU8            CircleSegmentCounts[64];    // Precomputed segment count for given radius (array index + 1) before we calculate it dynamically (to avoid calculation overhead)
    const Imvec3*   TexUvLines;                 // UV of anti-aliased lines in the atlas
...
struct ImGuiContext
{
    ...
    Imvec3                  ColorPickerRef;                     // Initial/reference color at the time of opening the color picker.
...
namespace ImGui
{
    ...
    IMGUI_API bool          ImageButtonEx(ImGuiID id, ImTextureID texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec2& padding, const Imvec3& bg_col, const Imvec3& tint_col);
