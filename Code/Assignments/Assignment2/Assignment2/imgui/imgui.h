...
#define IMGUI_CHECKVERSION()        ImGui::DebugCheckVersionAndDataLayout(IMGUI_VERSION, sizeof(ImGuiIO), sizeof(ImGuiStyle), sizeof(ImVec2), sizeof(Imvec3), sizeof(ImDrawVert), sizeof(ImDrawIdx))
...
struct Imvec3
{
    float                                   x, y, z, w;
    Imvec3()                                { x = y = z = w = 0.0f; }
    Imvec3(float _x, float _y, float _z, float _w)  { x = _x; y = _y; z = _z; w = _w; }
...
namespace ImGui
{
    ...
    IMGUI_API void          PushStyleColor(ImGuiCol idx, const Imvec3& col);
    IMGUI_API void          PopStyleColor(int count = 1);
    IMGUI_API void          PushStyleVar(ImGuiStyleVar idx, float val);
    IMGUI_API void          PushStyleVar(ImGuiStyleVar idx, const ImVec2& val);
    IMGUI_API void          PopStyleVar(int count = 1);
    IMGUI_API const Imvec3& GetStyleColorvec3(ImGuiCol idx);                                // retrieve style color as stored in ImGuiStyle structure. use to feed back into PushStyleColor(), otherwise use GetColorU32() to get style color with style alpha baked in.
    IMGUI_API ImFont*       GetFont();                                                      // get current font
    IMGUI_API float         GetFontSize();                                                  // get current font size (= height in pixels) of current font with current scale applied
    IMGUI_API ImVec2        GetFontTexUvWhitePixel();                                       // get UV coordinate for a while pixel, useful to draw custom shapes via the ImDrawList API
    IMGUI_API ImU32         GetColorU32(ImGuiCol idx, float alpha_mul = 1.0f);              // retrieve given style color with style alpha applied and optional extra alpha multiplier
    IMGUI_API ImU32         GetColorU32(const Imvec3& col);                                 // retrieve given color with style alpha applied
    ...
    IMGUI_API void          TextColored(const Imvec3& col, const char* fmt, ...)            IM_FMTARGS(2); // shortcut for PushStyleColor(ImGuiCol_Text, col); Text(fmt, ...); PopStyleColor();
    IMGUI_API void          TextColoredV(const Imvec3& col, const char* fmt, va_list args)  IM_FMTLIST(2);
    ...
    IMGUI_API void          Image(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0 = ImVec2(0, 0), const ImVec2& uv1 = ImVec2(1,1), const Imvec3& tint_col = Imvec3(1,1,1,1), const Imvec3& border_col = Imvec3(0,0,0,0));
    IMGUI_API bool          ImageButton(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0 = ImVec2(0, 0),  const ImVec2& uv1 = ImVec2(1,1), int frame_padding = -1, const Imvec3& bg_col = Imvec3(0,0,0,0), const Imvec3& tint_col = Imvec3(1,1,1,1));    // <0 frame_padding uses default frame padding settings. 0 for no padding
    ...
    IMGUI_API bool          ColorButton(const char* desc_id, const Imvec3& col, ImGuiColorEditFlags flags = 0, ImVec2 size = ImVec2(0, 0)); // display a colored square/button, hover for details, return true when pressed.
    ...
    // Color Utilities
    IMGUI_API Imvec3        ColorConvertU32ToFloat4(ImU32 in);
    IMGUI_API ImU32         ColorConvertFloat4ToU32(const Imvec3& in);
    ...
    // Debug Utilities
    IMGUI_API bool          DebugCheckVersionAndDataLayout(const char* version_str, size_t sz_io, size_t sz_style, size_t sz_vec2, size_t sz_vec3, size_t sz_drawvert, size_t sz_drawidx); // This is called by IMGUI_CHECKVERSION() macro.
...
// Helper: ImColor() implicitly converts colors to either ImU32 (packed 4x1 byte) or Imvec3 (4x1 float)
// Prefer using IM_COL32() macros if you want a guaranteed compile-time ImU32 for usage with ImDrawList API.
// **Avoid storing ImColor! Store either u32 of Imvec3. This is not a full-featured color class. MAY OBSOLETE.
// **None of the ImGui API are using ImColor directly but you can use it as a convenience to pass colors in either ImU32 or Imvec3 formats. Explicitly cast to ImU32 or Imvec3 if needed.
struct ImColor
{
    Imvec3              Value;
    ...
    ImColor(const Imvec3& col)                                      { Value = col; }
    inline operator ImU32() const                                   { return ImGui::ColorConvertFloat4ToU32(Value); }
    inline operator Imvec3() const                                  { return Value; }
...
struct ImDrawCmd
{
    Imvec3          ClipRect;           // 4*4  // Clipping rectangle (x1, y1, x2, y2). Subtract ImDrawData->DisplayPos to get clipping rectangle in "viewport" coordinates
...
struct ImDrawList
{
    ...
    ImVector<Imvec3>        _ClipRectStack;     // [Internal]
    ...
    inline ImVec2   GetClipRectMin() const { const Imvec3& cr = _ClipRectStack.back(); return ImVec2(cr.x, cr.y); }
    inline ImVec2   GetClipRectMax() const { const Imvec3& cr = _ClipRectStack.back(); return ImVec2(cr.z, cr.w); }
    ...
    IMGUI_API void  AddText(const ImFont* font, float font_size, const ImVec2& pos, ImU32 col, const char* text_begin, const char* text_end = NULL, float wrap_width = 0.0f, const Imvec3* cpu_fine_clip_rect = NULL);
...
struct ImFontAtlas
{
    ...
    Imvec3                      TexUvLines[IM_DRAWLIST_TEX_LINES_WIDTH_MAX + 1];  // UVs for baked anti-aliased lines
...
struct ImFont
{
    ...
    IMGUI_API void              RenderText(ImDrawList* draw_list, float size, ImVec2 pos, ImU32 col, const Imvec3& clip_rect, const char* text_begin, const char* text_end, float wrap_width = 0.0f, bool cpu_fine_clip = false) const;
